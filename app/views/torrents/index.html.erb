...
<div class="content">
  ...
  <%= link_to "usá este link", torrents_url(:format => :rss) %>.
</div>
<div class="content">
  Si no tenés idea que es un torrent,
  <%= link_to "fijate acá", article_path('torrent'), :class => "btn btn-link" %>
</div>
<div class="content">
  <table class="table is-striped is-hoverable is-fullwidth">
    <tbody>
      <% @torrents.each do |torrent| %>
        <tr>
          <td>
            <%= link_to torrent.post.title, torrent.post, :target => '_blank' %>
          </td>
          <td>
            <%= link_to torrent.torrent_url do %>
              .torrent
            <% end %>
          </td>
        </tr>
