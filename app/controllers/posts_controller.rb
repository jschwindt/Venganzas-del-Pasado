class PostsController < ApplicationController
  ...
  def show
    ...
    page = params[:page].present? ? params[:page] : comments_collection
      .page
      .per(VenganzasDelPasado::Application.config.comments_per_page)
      .total_pages
  ...
  def create
    ...
    if @post.save
      ...
      redirect_to(
        new_post_url,
        notice: "El post '#{@post.title}' se subió con éxito y pronto será revisado y, si corresponde, aprobado. Abajo tenés nuevamente el formulario por si querés cargar más programas."
      )
    else
