class RedirectsController < ApplicationController
  def index
    ...
    when %r{^(\d{4})/(\d{2})/(\d{2})/la-venganza-sera-terrible}
      ...
      redirect_to(
        "/posts/la-venganza-sera-terrible-del-#{Regexp.last_match(3)}-#{Regexp.last_match(2)}-#{Regexp.last_match(1)}",
    ...
    when %r{^(\d{4})/(\d{2})/(\d{2})}
      ...
      redirect_to(
        "/posts/#{Regexp.last_match(1)}/#{Regexp.last_match(2)}/#{Regexp.last_match(3)}",
        status: :moved_permanently
      )
    ...
    when %r{^(\d{4})/(\d{2})/page/(\d+)}
      ...
      redirect_to(
        "/posts/#{Regexp.last_match(1)}/#{Regexp.last_match(2)}?page=#{Regexp.last_match(3)}",
        status: :moved_permanently
      )
